...
// Set up the PDF.js worker — use unpkg CDN pinned to the exact installed version
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (activeTool === "eraser") {
      ...
      for (const ann of annotations) {
        ...
        if (ann.tool === "rectangle" && ann.x !== undefined && ann.y !== undefined) {
          if (
            coords.x >= ann.x && coords.x <= ann.x + (ann.width || 0) &&
      ...
      {textInputPos && (
        ...
        >
          <input
            ...
            onBlur={submitText}
            className="bg-transparent border border-primary outline-none px-1 text-foreground"
